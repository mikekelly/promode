...
| `chief-product-officer` | Crucial, hard-to-reverse *product* one-way doors — goal hierarchy, personas, kill/build; drafts for ratification (inherits the session's top tier) |
...
- **Marketing is a first-class lane with an owner.** The marketing layer of the traceability hierarchy has its own inherit-tier drafting seat (`chief-marketing-officer`) and execution rung (`senior-marketer`), mirroring the CTO/CPO and engineer lanes: the offer is the thing before the copy, every generated asset traces to real source material (never invented claims or testimonials), growth scales to net cash rather than per-ad-set ROAS, and platform playbooks are dated and re-verified against the live platform because they decay silently. The full set is the [Marketing section of the opinion register](plugins/promode/docs/opinion-register.md).
- **Drain the user's unknowns, not just the code's.** Clarifying outcomes opens by finding out where the user actually is — their experience with the problem and the codebase. When that reveals unfamiliar territory, the main agent runs a *blind-spot pass* — delegating exploration to surface the unknown unknowns and teaching the user enough to react — because an interview only drains unknowns someone already perceives. When words fail, it asks for a reference (source code beats a lossy description).
- **Ratification needs comprehension, not a rubber stamp.** Autonomous runs get a two-ended contract: an autonomy remit set upfront (which decisions the agent may settle, which it must surface), and — at the merge point — an *offered* explainer plus comprehension check over the decision risk the run accumulated that was never ratified. It's an offer the user can decline, it fires on unratified decision risk rather than raw volume, and it never pauses the run — it lives only at the ratification point.
- **Order plans for reaction.** A plan or design draft handed over for sign-off is a reaction surface, so it leads with the decisions the user is most likely to change — the data model, the interfaces, the user-facing flows — and buries the mechanical tail. Ordering the artifact for reaction is part of the deliverable.
