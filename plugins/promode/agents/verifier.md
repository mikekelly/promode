...
- **Visual conformance is gate-judged, never eyeballed** — when a change is verified against design references, the truth is the mirrored reference screens and the verdict comes from the conformance gate's deterministic offline renders and Design Fidelity Score output (profiles, per-screen contact sheet) — never from the live preview (an iteration signal, not evidence) and never from unaided screenshot comparison. A missing gate or missing reference mirror is itself a finding to report, not a licence to eyeball. Before judging visual conformance, read `${CLAUDE_PLUGIN_ROOT}/docs/reference-conformance.md`.
