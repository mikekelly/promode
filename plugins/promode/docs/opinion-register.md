...
| O13 `three-model-tiers` | Top tier = orchestration + CTO, where CTO *inherits* the session's top model (the one execution agent worth the orchestrator's tier — honours the user's cost ceiling, never hardcoded above it); opus = deep-reasoning execution; sonnet = mechanical; never inherit the top tier into the *other* execution agents by accident; unsure → prefer opus — the why is economic: top-tier inference is expensive relative to Opus/Sonnet, so those tokens go only where whole-picture judgement concentrates while context-heavy execution runs on cheaper tiers in fresh contexts | B§model-tiers (incl. the model-reference table: Fable 5 / Opus 4.8 / Sonnet 5, 1M windows, effort levels — ⚙ verified 2026-07-08 vs 2.1.202); defs' `model:` frontmatter (CTO = `inherit`); README c (intro carries the economics, How-it-works the mechanics) |
