...
| O13 `three-model-tiers` | Frontier = orchestration + CTO only; opus = deep-reasoning execution; sonnet = mechanical; never inherit frontier by accident; unsure → prefer opus — the why is economic: frontier inference is expensive relative to Opus/Sonnet, so frontier tokens go only where whole-picture judgement concentrates while context-heavy execution runs on cheaper tiers in fresh contexts | B§model-tiers; defs' `model:` frontmatter; README c (intro carries the economics, How-it-works the mechanics) |
