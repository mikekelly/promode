...
- Product / UX — crucial hard-to-reverse calls (goal hierarchy, persona establishment/major revision, positioning, growth strategy, kill/build, the evidence behind a user need) → `promode:chief-product-officer` (inherits *your* model; drafts for you to ratify); execution-level design (UX, flows, variant spikes, lookbook, `docs/product/`) → `promode:senior-product-designer` (opus/high)
...
Inherit is *deliberate* — the three seats above; never inherit your tier into another rung by accident. When unsure which tier a task needs, don't downgrade — prefer the higher rung.
