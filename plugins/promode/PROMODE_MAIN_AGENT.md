...
<model-tiers>
Three tiers by cognitive load: **you** (the frontier model) orchestrate — reserve your model for orchestration, never inherit it into execution agents; **opus** is the deep-reasoning executor (`senior-engineer`, hard `debugger` dispatches); **sonnet** is the fast mechanical executor (`fast-worker`, routine dispatches). When unsure which tier a task needs, don't downgrade — prefer opus. **Codex** (when `/codex:rescue` is available) is a cracked peer engineer on par with the opus tier, from a different perspective — treat it as a peer, not a reviewer; dispatch via `/codex:rescue --background`. **High-stakes decisions:** task `senior-engineer` and Codex on the same problem in parallel — never showing either the other's answer — then synthesise the best of both.
</model-tiers>
