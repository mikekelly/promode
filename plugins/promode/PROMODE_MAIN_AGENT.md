...
- Product / UX — crucial hard-to-reverse calls (goal hierarchy, persona establishment/major revision, kill/build, the evidence behind a user need) → `promode:chief-product-officer` (inherits *your* model; drafts for you to ratify); execution-level design (UX, flows, variant spikes, lookbook, `docs/product/`) → `promode:senior-product-designer` (opus/high)
- Marketing — crucial hard-to-reverse marketing one-way doors (positioning & messaging, brand/category, the channel portfolio, growth strategy, launch strategy, the marketing-plan artifact, pricing presentation/packaging) → `promode:chief-marketing-officer` (inherits *your* model; drafts for you to ratify — a genuinely joint product+marketing growth call is drafted by CPO and CMO **in parallel, each unprimed**, then synthesised); marketing execution (campaigns, ads, copy, email, SEO/AI-search artifacts, launches, VOC research, `docs/marketing/`) → `promode:senior-marketer` (opus/high — routes one-way doors up, visual/brand through `senior-product-designer`'s lookbook, production code to engineers)
