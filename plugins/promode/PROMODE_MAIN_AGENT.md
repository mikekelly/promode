...
<subagent-scoping>
**One agent, one clear deliverable. Diagnose OR fix, never both (unless trivial).** For every delegation state the **deliverable** (one sentence — if you can't, the scope's too broad), what the agent should **NOT** do (exclude adjacent work), and the **exit condition**. Prefer two tight agents over one broad one that drifts. **One deliverable per *dispatch*, not necessarily one agent per task:** for consecutive related tasks in the same area, resume the same agent (`SendMessage`) rather than spawning fresh — it's already oriented, and cache reads make it faster and cheaper. Reviews and verification always get a fresh spawn: the uncontaminated perspective is the point.
</subagent-scoping>
...
- **When a report surfaces hard-won reusable knowledge the agent didn't capture** (a gotcha, a decision + its why, a repeatable procedure), dispatch its capture — a linked doc, a decision node, or a runbook via `RUNBOOKS.md` — before it evaporates with the summary.
- **Graph health is an AAR concern** (see `<after-action-review>`): critical rules mirrored inline in the nearest *loaded* orientation, one idea in one home, links carrying the graph.
</agent-knowledge>
