...
- **Steer and resume with `SendMessage`.** A running background agent can be steered mid-flight (send the course-correction instead of killing and respawning); a completed/stopped one can be re-woken by its `agentId` (from the spawn result) with context intact. Prefer this when the follow-up builds on what the agent already holds; spawn fresh when you want an uncontaminated perspective (reviews, verification).
