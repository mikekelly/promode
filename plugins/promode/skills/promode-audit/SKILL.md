...
<claude-md-health>
`CLAUDE.md` is auto-loaded into **every** agent's context, so it's the highest-leverage file in the repo. The Agent-knowledge assessor evaluates it specifically against three tests, and recommends a concrete restructure (what to cut, what to link, what signposts to add) where it falls short. Two of these are *artifact* checks you can run by reading `CLAUDE.md` itself; the inline-criticality test (#2) is a **traversal** check — you cannot tell what's *missing* from `CLAUDE.md` by reading it alone, so walk the linked docs, ADRs, and load-bearing code/comments and ask of each crucial constraint you find: *is this surfaced inline here?*
...
2. **Critical knowledge is stated inline, never merely reachable by a link.** An agent acts only on what's already in its context, and **it won't follow links it doesn't know exist** — so anything an agent would *fail or do harm without* must live **in** `CLAUDE.md`, not one hop away. The rule is **criticality, not topic** (everything non-critical — detailed role-specific guidance for engineering, QA, product design, marketing — is demoted to the wiki and reached by a signpost link). This bites hardest for **crucial design constraints** — invariants, prohibitions, required patterns, load-bearing decisions — that habitually hide in ADRs, knowledge docs, or code comments: each one that's load-bearing, harmful-if-violated, and non-obvious from the code must appear **inline** (concisely, with a link to its full rationale), never be left as an indirect traversal an agent may never make. **A crucial constraint reachable only by traversal is the highest-severity finding in this dimension** — an agent can't honour a rule it was never shown — so rank it `Now` and recommend the **`reinforce-design-constraints`** skill, the write-action that hoists each buried constraint inline with its link. Also flag the inverse failure: non-critical detail bloating `CLAUDE.md` that belongs in a linked doc.
...
- **Now** = high impact, low/medium effort (unblocks agents working effectively — e.g. "no fast test loop", "no CLAUDE.md orientation", "a load-bearing constraint reachable only by a link an agent may never follow"). **Next** = high impact, higher effort. **Later** = lower impact / nice-to-have.
