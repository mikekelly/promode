...
This skill traverses the knowledge graph, finds the crucial design constraints, and makes sure each one is **guaranteed-loaded** into the nearest loaded orientation file that governs the affected area — stated **inline** (concisely), or **`@import`-transcluded** from that `CLAUDE.md` when the rule is too long to inline cleanly or lives in a shared doc — with a **link** to its full rationale so an agent who wants the whole story can discover it. The binding rule is in front of the affected agent (a plain link is not — the agent may never open it); the derivation stays one hop away.
...
6. **Reinforce.** Edit the chosen `CLAUDE.md` so each crucial constraint is inline in the format below, with a signpost link. If the concise rule itself is too long to inline (or it's a shared critical doc), **`@import` it from the governing `CLAUDE.md`** instead of a plain link — transclusion guarantees it loads. If the rationale doc doesn't exist yet, create it as a node and link it into the graph. **Never clobber** — integrate into the existing structure; append and link. If there's no root `CLAUDE.md`, create a minimal one as the root. Ensure every created or maintained orientation file has an adjacent `AGENTS.md -> CLAUDE.md` symlink where symlinks are supported; if not, duplicate minimally and note the lockstep requirement.
...
- **A plain link never carries a critical rule.** If a constraint must stay in its own file rather than inline, **`@import` it** (guaranteed transcluded into context), not `[link]` it (a pointer the agent may skip). Mind the locality cost: a *root* `@import` loads every session, so a rule critical only in a subtree belongs in that subtree's `CLAUDE.md` (auto-loaded on-interaction), not a root import.
