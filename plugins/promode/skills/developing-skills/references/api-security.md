...
<the_solution>
Use a secure API wrapper script that loads credentials internally.
...
# Examples:
scripts/secure-api.sh facebook list-campaigns
scripts/secure-api.sh ghl search-contact "email@example.com"
```
</for_supported_services>
...
3. **Add credential placeholders to your `.env` file** using profile naming:
...
1. **Check for saved profile:**
   ```bash
   scripts/profile-state get yourservice
   ```

2. **If no profile saved, discover available profiles:**
   ```bash
   scripts/list-profiles yourservice
   ```
...
5. **Save user's selection:**
   ```bash
   scripts/profile-state set yourservice <selected_profile>
   ```
...
```bash
scripts/secure-api.sh yourservice:<profile> <operation> [args]

# Examples:
scripts/secure-api.sh yourservice:main list-items
scripts/secure-api.sh yourservice:main get-item <ITEM_ID>
```
...
<credential_storage>
**Location:** `.env` file (configure as needed for your environment)
